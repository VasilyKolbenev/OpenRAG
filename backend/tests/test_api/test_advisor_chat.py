...
class TestAdvisorChat:
    ...
    async def test_recommendation_extraction(
        ...
        reply_with_rec = (
            "Based on your needs, I recommend GraphRAG.\n\n"
            "```json\n"
            '{"recommended": "graph", "scores": {"graph": 0.9, '
            '"lightrag": 0.6}, "reasoning": "High accuracy needed for medical",'
            ' "settings": {"max_hops": 4}}\n'
            "```"
        )
        ...
        with patch("app.api.v1.advisor.litellm") as mock_litellm:
            mock_response = MagicMock()
            mock_response.choices = [MagicMock(message=MagicMock(content=reply_with_rec))]
            mock_litellm.acompletion = AsyncMock(return_value=mock_response)
        ...
        assert data["recommendation"]["recommended"] == "graph"
    ...
    async def test_llm_error_graceful_fallback(
        ...
        with patch("app.api.v1.advisor.litellm") as mock_litellm:
            ...
            response = await client.post("/advisor/chat", json={"message": "Hello"})

        assert response.status_code == 200
        data = response.json()
        assert "LightRAG" in data["reply"]
...
class TestExtractRecommendation:
    ...
    def test_extracts_inline_json(self):
        reply = 'I recommend: {"recommended": "lightrag", "scores": {"lightrag": 0.9}}'
        rec = _extract_recommendation(reply)
        assert rec is not None
        assert rec.recommended == "lightrag"
