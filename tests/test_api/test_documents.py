...
class TestDocumentUpload:
    ...
    async def test_upload_valid_text_file(self, client: AsyncClient, app):
        # The endpoint does: from openrag.workers.tasks.ingest import process_document_task
        ...
        with (
            patch("openrag.api.v1.documents.os.makedirs"),
            patch("openrag.api.v1.documents.os.path.realpath", side_effect=lambda p: p),
            patch("openrag.api.v1.documents.aiofiles.open", return_value=mock_ctx),
